...
**Authority delegated-domain configuration storage**: Authority delegated-domain configuration values (which `ai_executive` actors have which delegated scope) are stored as structured JSON in an AMC-owned Supabase configuration table `authority_domain_config` (not environment variables, as this is runtime-manageable persistent configuration state). AMC API middleware reads this table on each authority check for `delegated` boundary types. `authority_domain_config` is part of the canonical AMC-owned table set and AMC state ownership model and must therefore be treated as tenant-isolated security-sensitive state: every record is scoped by `organisation_id`; Row Level Security must prevent cross-tenant reads and writes; normal application reads are limited to AMC authority-check middleware and explicitly authorised AMC backend services acting within the same tenant context; writes are restricted to privileged AMC authority-configuration administration paths only. This table is append-only from an audit perspective — changes are new records, not in-place updates — and no direct end-user write path is permitted. This decision is made at Architecture stage (was deferred from TRS §25).
