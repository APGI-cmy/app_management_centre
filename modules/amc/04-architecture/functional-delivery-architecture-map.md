...
**Authority basis**: PR #1186, FR-1900, TR-1900, Stage 2 CTA/API/Data/Audit matrix  
...
| Knowledge-Aware View | `/knowledge` | Retrieve knowledge with provenance and stale-state display | TR-701 to TR-703; Architecture §5.4/§6.3 | CLEAN — read-only projection and provenance required |
...
| KUC upload submission | TRS-defined outbound KUC submission contract | `knowledge_upload_records` projection, `audit_events` | `KUC_UPLOAD_SUBMITTED` or Stage 5/6 canonical equivalent | KUC unavailable shown; no direct AIMCC ingestion | CLEAN |
| AIMCC status/quota read | TRS-defined AIMCC read/status contract | `knowledge_upload_records`, quota projection | AIMCC status/quota view events where audit applies | Stale projection and source timestamp required | CLEAN |
| Quota adjustment / override | `POST /api/aimcc/quota/request-adjustment` | `approvals`, quota projection, `audit_events` | `QUOTA_ADJUSTMENT_REQUESTED`, `AIMCC_GOVERNANCE_ACTION_CREATED`, override lifecycle events | Approval gate; temporary override via `adjustment_type` | CLEAN |
| Knowledge retrieval | `POST /api/knowledge/query` | transient display cache, retrieval log | `KNOWLEDGE_RETRIEVED` | Provenance required; stale/TTL marker required | CLEAN |
...
| Mobile critical alert action | Alert API family through responsive/mobile surface | `alerts`, `audit_events` | mobile alert open/action events | Mobile action parity and cross-device consistency | CLEAN |
...
| KUC | Governed upload submission path | submission rejected/unavailable | Show receipt or rejection/unavailable state | KUC submission/failure event |
| Knowledge / Memory | Read-only knowledge/provenance source | stale/provenance unavailable | Do not present stale output as current truth | retrieval log with provenance/stale marker |
| Foreman | Intervention dispatch / governed execution target | dispatch failed / callback stale | Show failed dispatch or stale execution timeline | dispatch/callback/failure audit events |
| Specialist agents | Execution status source for delegated work | callback unavailable/stale | Show execution stale/failure state | authenticated callback event |
...
| Stage 6 QA derivation | Stage 6 must import dead-CTA/functional-delivery tests | Require Stage 6 retrofit/disposition after Stage 5 |
