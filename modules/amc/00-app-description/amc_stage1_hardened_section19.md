...
AMC shall require an explicit table pathway audit so that persisted data structures, storage flows, and state-bearing records remain understandable, auditable, and faithful to the executive operating model of the estate.

This section exists because AMC is intended to become the executive operating centre of the Maturion estate. In such a system, persistent data is not merely technical plumbing. Tables, collections, records, and equivalent storage pathways may carry approval state, escalation state, intervention state, audit linkage, identity context, sandbox boundaries, conversation continuity, and executive operating truth. If these pathways are allowed to emerge informally, the system may look coherent at the interface level while its underlying data model quietly undermines authority, continuity, or reviewability.

For AMC, table pathway audit is therefore a governance-protective design discipline, not just a database housekeeping exercise.

### Purpose of the Table Pathway Audit
The AMC table pathway audit shall exist to ensure that every material persisted pathway can be understood in terms of:
- what it stores
- why it exists
- which product meaning it carries
- who or what writes to it
- who or what reads from it
- what authority boundaries apply
- how it relates to audit, approval, state, and continuity requirements

The audit must make it possible to detect storage-level ambiguity before that ambiguity hardens into runtime misbehavior or governance-blind persistence.

### Core Principle
No materially significant persisted pathway in AMC may be treated as “just storage.”

Every important table or equivalent persisted structure must have:
- a clear purpose
- a clear owning meaning
- a clear state relationship
- a clear access pattern
- a clear boundary model
- a clear audit implication

If a table exists but its product meaning cannot be explained, the design is not ready.

### Scope of the Audit
The table pathway audit applies to all persisted pathways that materially affect AMC behavior.
...
- audit-linked reference tables
- identity or role metadata tables
- sandbox and bounded-environment records
- state-restoration records
- mapping or junction tables whose semantics affect access or state meaning

The term “table” in this section includes any equivalent persisted structure used by the final technical implementation.

### Upstream Truth Coupling
Table pathway audit must derive from approved upstream truth.

Persisted pathways must be traceable to:
- the AMC App Description
- approved FRS requirements
- approved TRS requirements
- approved Architecture decisions
- approved state, auth, audit, AI, or notification design artifacts where relevant

A persisted pathway that exists only because it was technically convenient, but has no traceable product meaning, is a design-governance defect.

### Purpose and Ownership Requirement
Each material table or persisted pathway must have a clear declared purpose.

For each such pathway, the design should be able to answer:
- what business or operating meaning it carries
- what state domain it belongs to
- which workflows depend on it
- whether it is authoritative, derived, historical, or transitional
- whether it is sandbox-bound, tenant-bound, actor-bound, or estate-wide
- who or what is responsible for populating or maintaining it

A table whose purpose is undefined or whose ownership meaning is vague is not acceptable for AMC.

### State-Domain Mapping Requirement
Persisted pathways must be explicitly mapped to AMC’s major state domains.

This includes, where relevant, mapping to:
...
- sandbox and bounded-environment state
- audit-linked state references
- persistence and restoration state

This mapping is necessary to prevent accidental duplication, semantic overlap, or missing storage responsibilities across the product.

### Authoritative vs Derived Data Requirement
The table pathway audit must distinguish between authoritative and derived persisted data.

For each important pathway, the design must clarify:
- whether it is a source of truth
- whether it is a materialized or derived convenience view
- whether it is a cache-like structure
- whether it is historical-only
- whether it is operationally live

AMC must not allow derived or convenience structures to be mistaken for constitutional or operational truth without explicit design intent.

### Write Pathway Requirement
The audit must define who or what can write to each material persisted pathway.

This includes identifying:
- user-initiated writes
- AI-initiated writes
- service-initiated writes
- background or orchestration writes
- approval-path writes
- audit-event writes
- sandbox-local writes
- cross-boundary or escalated writes where relevant

Write pathways must be reviewable because write capability is often where hidden authority expansion occurs.

### Read Pathway Requirement
The audit must also define who or what can read each material persisted pathway.

This includes clarifying:
- which users or roles may read the data
- which AI or agent classes may read the data
- whether the data is executive-only, assurance-visible, maintenance-visible, or sandbox-restricted
- whether summary views, detail views, and audit views are reading from the same or different sources
- how sensitive or boundary-restricted data is prevented from leaking across contexts

Read behavior shapes executive truth just as much as write behavior does.

### Approval and Authority Pathway Requirement
Any persisted pathway carrying approval, authority, or reserved-matter significance must be audited with special rigor.

The table pathway audit must confirm that relevant structures can faithfully represent:
- pending vs approved vs rejected vs deferred vs expired states
- approving authority identity
- acting identity where different
- scope of approval
- linkages to intervention or execution behavior
- invalidation or supersession of earlier approval where relevant

A table design that makes approval state ambiguous is non-compliant with AMC’s operating model.

### Audit and Event-Linkage Requirement
Where persisted data participates in consequential actions, the audit must confirm that audit linkage is structurally possible and semantically clear.

This includes verifying that:
- approvals can be linked to audit records
- interventions can be linked to audit records
- escalations can be linked to audit records
- AI-mediated consequential writes remain attributable
- state transitions can be understood historically where needed

Persisted pathways that sever the connection between operational truth and audit truth are not fit for AMC.

### Notification and Escalation Pathway Requirement
Persisted pathways for notifications and escalations must preserve signal meaning and lifecycle meaning.

This includes confirming that the storage model can represent:
- notification class
- severity
- issuance time
- routing target
...
- dismissal, closure, or follow-up state
- relationship to the underlying issue or event

Loose event dumping without lifecycle meaning is insufficient for AMC.

### Intervention and Execution Pathway Requirement
Where AMC supervises governed interventions or jobs, persisted pathways must preserve operational lifecycle meaning.

The audit should confirm that the storage model can represent:
- intervention proposal
- approval linkage
- initiation
- orchestration linkage
- in-flight status
- failure, rollback, or completion
- relationship to underlying estate issue or executive decision

A persistent model that only records fragments of this lifecycle is operationally weak.

### Conversation and Continuity Pathway Requirement
Where conversation continuity or executive context is persisted, the table pathway audit must ensure that persistence meaning remains controlled.

This includes confirming:
- what conversational state is stored
- whether action-relevant conversational context can be distinguished from ephemeral dialogue
- how executive context is restored
- how stale conversation fragments are prevented from masquerading as current truth
- how cross-device continuity is supported without uncontrolled state resurrection

Conversation persistence without pathway clarity can create false continuity and governance confusion.

### Sandbox and Boundary Requirement
Persisted pathways that touch sandboxed or bounded environments must preserve those boundaries structurally.

The audit must ensure that:
- sandbox identity is representable
- boundary context is not lost
- sandbox-local records are distinguishable from estate-wide records
- cross-boundary proposals are distinguishable from direct cross-boundary writes
- specialist-bound activity does not silently become estate-level truth

A sandbox model is only real if its persisted pathways preserve its boundaries.

### Duplication and Semantic Drift Check
The table pathway audit must look for duplication and semantic drift across persisted structures.

This includes checking for:
- multiple tables that appear to represent the same truth with different semantics
- approval or status meaning duplicated inconsistently
- historical and current state mixed without clarity
- summary structures drifting away from authoritative records
- convenience writes that bypass intended pathways

AMC cannot sustain coherent executive truth if storage-level semantics are drifting underneath the surface.

### Retention and Lifecycle Requirement
Persisted pathways must have understood retention and lifecycle behavior.

For each important pathway, downstream design should define:
- whether the data is transient, medium-lived, or long-lived
- whether it should be archived, rotated, or retained indefinitely
- whether it participates in operational restoration
- whether it participates in historical audit or assurance review
- when it may be safely superseded or invalidated

Persistence without lifecycle discipline produces clutter at best and governance ambiguity at worst.

### Validation Evidence Requirement
Table pathway audit must be evidenced.

Evidence may include, as appropriate:
- table/pathway inventories
- schema review notes
- data-flow diagrams
- source-of-truth mappings
- write/read pathway notes
- review comments
- issue records for ambiguous or duplicated pathways
- approval notes on resolved storage-meaning questions

The exact evidence form may vary, but materially significant storage pathways must not remain undocumented in meaning.

### Minimum Table Pathway Audit Checklist
The following minimum conditions must hold for materially significant persisted pathways:

- [ ] Material tables or equivalent persisted pathways are inventoried
- [ ] Each pathway has a defined purpose and ownership meaning
- [ ] Pathways are mapped to major AMC state domains
- [ ] Authoritative vs derived pathways are distinguished
- [ ] Write pathways are identified
- [ ] Read pathways are identified
- [ ] Approval and authority-bearing pathways are structurally clear
- [ ] Audit linkage is structurally possible where required
- [ ] Notification and escalation pathways preserve lifecycle meaning
- [ ] Intervention and execution pathways preserve lifecycle meaning
- [ ] Conversation and continuity pathways are controlled where relevant
- [ ] Sandbox and boundary semantics are preserved where relevant
- [ ] Duplication and semantic drift risks have been considered
- [ ] Retention and lifecycle expectations are acknowledged
- [ ] Evidence of pathway audit exists

### Downstream Design Consequence
Downstream TRS, Architecture, and data/state design artifacts must explicitly identify AMC’s material persisted pathways and show how each one satisfies this audit discipline.

Table pathway audit may not be postponed until late database review. It must be part of the structural design of the build.

### Non-Negotiable Principle
AMC table pathway audit must ensure that persisted storage pathways preserve the approved meaning of the executive operating environment rather than silently distorting it.

Any design that leaves important approval, authority, audit, continuity, or boundary-carrying data pathways semantically vague, duplicated, or weakly governed is non-compliant with the intended purpose of AMC.
