...
| **SURF-003** | DB migration | `db-migrate.yml` | Manual dispatch only | Production (Supabase production project); Staging (Supabase staging project) | ✅ `production` environment gate — CS2 approval required (production only); no gate for staging | GitHub-hosted (`ubuntu-latest`) | Supabase CLI (`supabase db push --project-ref $SUPABASE_PROJECT_REF`) is the only approved mechanism. Target project is selected via `--project-ref` from the per-environment secret — no `supabase link` step required. See DES-004. |
