...
| Dynamic Upload Quota Console | Request quota adjustment | `POST /api/aimcc/quota/request-adjustment` | `approvals`, `audit_events`, quota projection | AIMCC after approval | `AIMCC_GOVERNANCE_ACTION_CREATED` | Reserved matter approval required; reason and requested value required | Approval item created; quota not changed before approval | Stage 6 test: quota change is approval-gated |
| Dynamic Upload Quota Console | Temporary quota override | `POST /api/aimcc/quota/temporary-override` | `approvals`, `audit_events`, quota projection | AIMCC | `QUOTA_OVERRIDE_REQUESTED` | Expiry required; approval required; pre-expiry alert required | Override pending/active/expired state visible | Stage 6 test: override expiry and approval gate enforced |
...
| ARC Governance Console | Review ARC trigger | `GET /api/arc/triggers/{id}` | `arc_triggers`, `approvals`, `audit_events` | ARC source / Foreman where applicable | `ARC_TRIGGER_VIEWED` | Reserved/constitutional triggers route to CS2 approval boundary | Trigger detail shows authority boundary and next action | Stage 6 test: ARC trigger authority boundary visible |
| ARC Governance Console | Decide ARC trigger action | `POST /api/arc/triggers/{id}/decide` | `arc_triggers`, `approvals`, `interventions`, `audit_events` | Foreman if intervention dispatched | `ARC_TRIGGER_DECIDED` | Reserved action requires approval; downstream dispatch failure visible | Decision outcome and created approval/intervention reference shown | Stage 6 test: ARC decision persists and dispatch is not assumed |
