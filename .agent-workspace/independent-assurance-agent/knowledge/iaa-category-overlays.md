...
| OVL-PBG-012 | Stage artifact existence | The canonical artifact(s) for each declared stage must be present in the PR diff or confirmed on the branch. The expected artifact per stage is defined in `PRE_BUILD_STAGE_MODEL_CANON.md` v1.0.0 §3.1. Missing artifact for any declared stage = REJECTION-PACKAGE citing the specific stage and expected artifact. |
