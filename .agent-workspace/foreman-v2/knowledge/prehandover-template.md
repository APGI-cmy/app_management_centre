...
<!-- §4.3b (AGENT_HANDOVER_AUTOMATION.md v1.1.3): PREHANDOVER proof is READ-ONLY after initial commit.
     ...
     After IAA verdict, IAA records PHASE_B_BLOCKING_TOKEN in wave record section 5 (AMC 90/10 v1.0.0 — no standalone token file).
     Do NOT edit this file post-commit. -->
`iaa_audit_token: IAA-session-NNN-waveY-YYYYMMDD-PASS`
<!-- A-030: Token date MUST match actual IAA session date. Read PHASE_B_BLOCKING_TOKEN from wave record section 5 BEFORE writing this field. Do NOT use session date. For multi-batch waves, the token date may differ from the session date. -->
...
4. **Post-commit (wave record section 5 only)**: IAA records `PHASE_B_BLOCKING_TOKEN: IAA-session-NNN-YYYYMMDD-PASS` in wave record section 5. No standalone token file. No amendments to the committed PREHANDOVER proof.
