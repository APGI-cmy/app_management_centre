...
| A-036 | GOVERNING-ISSUE-PARITY-MANDATORY (CS2 — 2026-04-23): Before issuing any QP PASS verdict, the Foreman MUST execute the Governing-Issue Parity Check defined in `governance/canon/GOVERNING_ISSUE_PARITY_PROTOCOL.md` (GIPC-001) §2. The same Stage Kickoff Issue number MUST appear as the governing authority across all surfaces in the GIPC-001 §2.2 artifact chain: PR body, wave record triggering_issue, wave checklist authority, main artifact header, PREHANDOVER proof, and session memory. If any surface cites a different issue (hardening, harmonization, approval reference, or CI issue), the QP verdict MUST be FAIL and the wave must be corrected before re-evaluation. Parity check evidence (GIPC-001 §2.4 block) MUST be present in the PREHANDOVER proof. A QP PASS without parity check evidence is a documentation fabrication. Violation class: GIP-PARITY-FAIL. | CS2 — app_management_centre#1129 (2026-04-23) |
