...
| ACR-15 | **Active-wave / tracker contradiction rejection** — the `wave-current-tasks.md` checklist for the active wave (most recent file in `.agent-admin/waves/`) declares a wave, session, or job identifier that contradicts the wave/session/job identifier declared in the active PREHANDOVER proof (`wave_id` field), wave record, or session memory; or the `wave-current-tasks.md` is present but references a different wave slug from any of the other active-bundle artifacts | AGENT_HANDOVER_AUTOMATION.md §4.3e Check L; AAP-22 |
